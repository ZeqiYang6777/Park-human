...
/* Version: 1.1.63  */
/* Exported on: 28.05.2025 23:05:42 */
	...
	SubShader
	{
		...
		Pass
		{
			...
			inline void MKPostProcessing(in float2 uv, in MKUserData userData, out half4 result)
			{
				...
				float noiseOffset = 8 + position.x + position.y + position.z;
				
				#ifdef MK_SKETCH_ADDITIONAL_NOISE_MAP
					const half noise = SampleTex2DNoScale(PASS_TEXTURE_2D(_MKEdgeDetectionSketchNoiseMap, MK_SAMPLER_DEFAULT), noiseOffset.xx);
				#else
					const half noise = 0.5h;
				#endif
				
				float2 positionFalloff = clamp(noiseOffset + noise, -100, 100);
				float2 uvOffset = sin(positionFalloff * _SketchParams.y) * _SketchParams.x * depthFade;
				half3 edgeMask = SampleTex2DAuto(PASS_TEXTURE_2D(_MKEdgeDetectionEdgeMask, MK_SAMPLER_LINEAR_CLAMP), uvOffset + uv).rgb;
				//edgeMask += SampleTex2DAuto(PASS_TEXTURE_2D(_MKEdgeDetectionEdgeMask, MK_SAMPLER_LINEAR_CLAMP), -uvOffset + uv).rgb;
